...
const ELIMINAR_SUCURSAL = gql`
    mutation EliminarSucursal($eliminarSucursalId: ID!) {
        eliminarSucursal(id: $eliminarSucursalId)
    }
`;
...
const Sucursal = ({sucursal}) => {
    const {id, nombre, direccion} = sucursal;
    ...
    const [eliminarSucursal] = useMutation(ELIMINAR_SUCURSAL, {
        update(cache) {
            ...
            cache.writeQuery({
                ...
                data: {
                    obtenerSucursalesBanco: obtenerSucursalesBanco
                        .filter(sucursalActual => sucursalActual.id !== id)
                }
            });
        }
    });
